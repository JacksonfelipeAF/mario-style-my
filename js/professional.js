...
const playSound = (type, volume = 1) => {
  ...
  switch(type) {
    case 'jump':
      ...
      gainNode.gain.setValueAtTime(0.3 * adjustedVolume, audioContext.currentTime);
    ...
    case 'coin':
      ...
      gainNode.gain.setValueAtTime(0.2 * adjustedVolume, audioContext.currentTime);
      gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + 0.1);
      oscillator.start(audioContext.currentTime);
      oscillator.stop(audioContext.currentTime + 0.1);
      break;
    ...
    case 'powerup':
      ...
      gainNode.gain.setValueAtTime(0.4 * adjustedVolume, audioContext.currentTime);
      gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + 0.3);
      oscillator.start(audioContext.currentTime);
      oscillator.stop(audioContext.currentTime + 0.3);
      break;
    ...
    case 'gameOver':
      ...
      gainNode.gain.setValueAtTime(0.5 * adjustedVolume, audioContext.currentTime);
...
class AchievementSystem {
  ...
  saveAchievements() {
    localStorage.setItem('marioAchievements', JSON.stringify(this.achievements));
  }
...
const showMainMenu = () => {
  ...
  document.getElementById('total-played').textContent = gameState.totalGamesPlayed;
  ...
  loop = setInterval(() => {
    ...
    if (!gameState.isInvincible) {
      if (pipePosition <= 50 && pipePosition > 0 && marioPosition < 90) {
        loseLife();
      } else if (obstaclePosition <= 50 && obstaclePosition > 0 && marioPosition < 90) {
        loseLife();
      }
    }
...
const saveSettings = () => {
  ...
  localStorage.setItem('marioSettings', JSON.stringify({
    difficulty,
    volume,
    quality
  }));
...
const loadSettings = () => {
  ...
  if (settings) {
    ...
    document.getElementById('quality-select').value = parsed.quality || 'medium';
  }
};
